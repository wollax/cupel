...
- [ ] M008: Rust OpenTelemetry Bridge — `cupel-otel` companion crate bridging `TraceCollector` to `opentelemetry` API; 3 verbosity tiers (StageOnly, StageAndExclusions, Full); `cupel.pipeline` / `cupel.stage.*` spans; parity with `Wollax.Cupel.Diagnostics.OpenTelemetry`; R058 to be validated (S01 complete: `on_pipeline_completed` hook + `StageTraceSnapshot` wired into core; 170 tests pass)
