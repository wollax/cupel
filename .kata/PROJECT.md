...
- [ ] M006: Count-Based Quotas — Implement `CountQuotaSlice` in both Rust and .NET: two-phase count-satisfy + budget-distribute algorithm, `CountCapExceeded` exclusion reason, `count_requirement_shortfalls` on SelectionReport, scarcity degradation, construction-time guards (KnapsackSlice rejection, require > cap)
