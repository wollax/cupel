...
- [x] M008: Rust OpenTelemetry Bridge — `cupel-otel` companion crate; `CupelOtelTraceCollector` implements `TraceCollector`; 3 verbosity tiers (StageOnly, StageAndExclusions, Full); `cupel.pipeline` / `cupel.stage.*` spans with exact `cupel.*` attributes; `TraceCollector::on_pipeline_completed` hook + `StageTraceSnapshot` in core; `cargo package --no-verify` exits 0; spec `## Rust (cupel-otel)` section; R058 validated (all 3 slices complete, 2026-03-24; 5 integration tests passing, 167 Rust tests)
- [ ] M009: CountConstrainedKnapsackSlice + MetadataKeyScorer — `CountConstrainedKnapsackSlice` slicer (pre-processing path: phase 1 commits count-required items by score, phase 2 runs standard KnapsackSlice on residual budget) in both Rust and .NET; `MetadataKeyScorer` multiplicative scorer with `cupel:priority` convention; spec chapters for both
