...
- [ ] M008: Rust OpenTelemetry Bridge — `cupel-otel` companion crate bridging `TraceCollector` to `opentelemetry` API; 3 verbosity tiers (StageOnly, StageAndExclusions, Full); `cupel.pipeline` / `cupel.stage.*` spans; parity with `Wollax.Cupel.Diagnostics.OpenTelemetry`; R058 to be validated (S01 ✓: `on_pipeline_completed` hook + `StageTraceSnapshot`; S02 ✓: full `CupelOtelTraceCollector` with all 3 tiers, 5 integration tests passing; S03 next: packaging + spec + CHANGELOG)
